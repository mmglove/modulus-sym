...
def test_advection_diffusion():
    ...
    evaluations = eq.make_nodes()[0].evaluate(
        {
            "T__t": paddle.to_tensor(T__t, dtype="float32"),
            "T__x": paddle.to_tensor(T__x, dtype="float32"),
            "T__y": paddle.to_tensor(T__y, dtype="float32"),
            "T__z": paddle.to_tensor(T__z, dtype="float32"),
            "T__x__x": paddle.to_tensor(T__x__x, dtype="float32"),
            "T__y__y": paddle.to_tensor(T__y__y, dtype="float32"),
            "T__z__z": paddle.to_tensor(T__z__z, dtype="float32"),
