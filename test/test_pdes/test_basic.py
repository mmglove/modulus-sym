...
def test_curl():
    ...
    evaluations_u = curl_eq.make_nodes()[0].evaluate(
        {
            "c__y": paddle.to_tensor(c__y, dtype="float32"),
            "b__z": paddle.to_tensor(b__z, dtype="float32"),
        }
    )
    evaluations_v = curl_eq.make_nodes()[1].evaluate(
        {
            "a__z": paddle.to_tensor(a__z, dtype="float32"),
            "c__x": paddle.to_tensor(c__x, dtype="float32"),
        }
    )
    evaluations_w = curl_eq.make_nodes()[2].evaluate(
        {
            "b__x": paddle.to_tensor(b__x, dtype="float32"),
            "a__y": paddle.to_tensor(a__y, dtype="float32"),
        }
    )
