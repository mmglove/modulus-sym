...
class IntegralBoundaryConstraint(IntegralConstraint):
    ...
    def __init__(
        ...
        if fixed_dataset:
            ...
            # list_invar = [
                # dict(np.load(f"/workspace/hesensen/modulus_pd_th_bkd_compare/modulus-sym/examples/chip_2d/outputs/cuda_graphs=false,graph.func_arch=false,graph.func_arch_allow_partial_hessian=false,jit=false,jit_use_nvfuser=false_torch_solid_solid全量训练版/chip_2d_solid_solid_heat_transfer/{loss.name}/list_invar_torch[{i}].npz"))
            #     for i in range(len(list_invar))
            # ]
            # list_outvar = [
                # dict(np.load(f"/workspace/hesensen/modulus_pd_th_bkd_compare/modulus-sym/examples/chip_2d/outputs/cuda_graphs=false,graph.func_arch=false,graph.func_arch_allow_partial_hessian=false,jit=false,jit_use_nvfuser=false_torch_solid_solid全量训练版/chip_2d_solid_solid_heat_transfer/{loss.name}/list_outvar_torch[{i}].npz"))
            #     for i in range(len(list_outvar))
            # ]
            # list_lambda_weighting = [
                # dict(np.load(f"/workspace/hesensen/modulus_pd_th_bkd_compare/modulus-sym/examples/chip_2d/outputs/cuda_graphs=false,graph.func_arch=false,graph.func_arch_allow_partial_hessian=false,jit=false,jit_use_nvfuser=false_torch_solid_solid全量训练版/chip_2d_solid_solid_heat_transfer/{loss.name}/list_lambda_weighting_torch[{i}].npz"))
